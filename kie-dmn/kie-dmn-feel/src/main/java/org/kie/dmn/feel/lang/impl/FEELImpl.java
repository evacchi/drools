...
public class FEELImpl
    ...
    public CompiledExpression compile(String expression, CompilerContext ctx) {
        ...
        ProcessedExpression processedExpression =
                new ProcessedExpression(
                        expression,
                        getEventsManager(listeners),
                        ctx,
                        profiles);
        if (doCompile || ctx.isDoCompile()) {
            return processedExpression.getCompiled();
        } else { // "legacy" interpreted AST compilation:
            return processedExpression.getInterpreted();
        }
    }
    ...
    public Object evaluate(CompiledExpression expr, Map<String, Object> inputVariables) {
        ExecutableExpression e = (ExecutableExpression) expr;
        return e.evaluate(newEvaluationContext(Collections.EMPTY_SET, inputVariables));
    }
    
    @Override
    public Object evaluate(CompiledExpression expr, EvaluationContext ctx) {
        ExecutableExpression e = (ExecutableExpression) expr;
        return e.evaluate(newEvaluationContext(ctx.getListeners(), ctx.getAllValues()));
    }
